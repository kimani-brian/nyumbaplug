...
type Repository interface {
	...
	GetAllLandlordProfiles(ctx context.Context) ([]domain.AgentView, error)
...
func (r *PostgresRepo) CreateUser(ctx context.Context, u *domain.User) error {
	...
	_, err := r.db.ExecContext(ctx, query, u.ID, u.Role, phone, u.Email, u.PasswordHash, u.CreatedAt)
	return err
}
...
func (r *PostgresRepo) CreateLandlordProfile(ctx context.Context, lp *domain.LandlordProfile) error {
	query := `INSERT INTO landlord_profiles (id, user_id, full_name, national_id_number, id_document_url, is_caretaker, authorized_by_landlord_id, verification_status, created_at) VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9)`
	_, err := r.db.ExecContext(ctx, query, lp.ID, lp.UserID, lp.FullName, lp.NationalIDNumber, lp.IDDocumentURL, lp.IsCaretaker, lp.AuthorizedByLandlordID, lp.VerificationStatus, lp.CreatedAt)
	return err
}
...
func (r *PostgresRepo) GetLandlordProfileByID(ctx context.Context, id uuid.UUID) (*domain.LandlordProfile, error) {
	...
	query := `SELECT id, user_id, full_name, national_id_number, id_document_url, is_caretaker, authorized_by_landlord_id, verification_status, verified_by, verified_at, revoked_at, revoke_reason, created_at FROM landlord_profiles WHERE id = $1`
	err := r.db.QueryRowContext(ctx, query, id).Scan(&lp.ID, &lp.UserID, &lp.FullName, &lp.NationalIDNumber, &lp.IDDocumentURL, &lp.IsCaretaker, &lp.AuthorizedByLandlordID, &lp.VerificationStatus, &lp.VerifiedBy, &lp.VerifiedAt, &lp.RevokedAt, &lp.RevokeReason, &lp.CreatedAt)
	if errors.Is(err, sql.ErrNoRows) {
...
func (r *PostgresRepo) GetLandlordProfileByUserID(ctx context.Context, userID uuid.UUID) (*domain.LandlordProfile, error) {
	lp := &domain.LandlordProfile{}
	query := `SELECT id, user_id, full_name, national_id_number, id_document_url, is_caretaker, authorized_by_landlord_id, verification_status, verified_by, verified_at, revoked_at, revoke_reason, created_at FROM landlord_profiles WHERE user_id = $1`
	err := r.db.QueryRowContext(ctx, query, userID).Scan(&lp.ID, &lp.UserID, &lp.FullName, &lp.NationalIDNumber, &lp.IDDocumentURL, &lp.IsCaretaker, &lp.AuthorizedByLandlordID, &lp.VerificationStatus, &lp.VerifiedBy, &lp.VerifiedAt, &lp.RevokedAt, &lp.RevokeReason, &lp.CreatedAt)
	if errors.Is(err, sql.ErrNoRows) {
		return nil, domain.ErrLandlordNotFound
	}
	return lp, err
...
func (r *PostgresRepo) GetLandlordProfilesByStatus(ctx context.Context, status string) ([]domain.LandlordProfile, error) {
	query := `SELECT id, user_id, full_name, national_id_number, id_document_url, is_caretaker, authorized_by_landlord_id, verification_status, verified_by, verified_at, revoked_at, revoke_reason, created_at FROM landlord_profiles WHERE verification_status = $1 ORDER BY created_at DESC`
	...
	for rows.Next() {
		...
		if err := rows.Scan(&lp.ID, &lp.UserID, &lp.FullName, &lp.NationalIDNumber, &lp.IDDocumentURL, &lp.IsCaretaker, &lp.AuthorizedByLandlordID, &lp.VerificationStatus, &lp.VerifiedBy, &lp.VerifiedAt, &lp.RevokedAt, &lp.RevokeReason, &lp.CreatedAt); err != nil {
			return nil, err
		}
		list = append(list, lp)
	}
...
func (r *PostgresRepo) CreateCategory(ctx context.Context, cat *domain.UnitCategory) error {
	...
	_, err := r.db.ExecContext(ctx, query, cat.ID, cat.PropertyID, cat.Name, cat.Description, cat.RentAmount, cat.QuantityAvailable, cat.Photos, cat.VideoURL, cat.CreatedAt)
	return err
}
...
func (r *PostgresRepo) GetCategoryByID(ctx context.Context, id uuid.UUID) (*domain.UnitCategory, error) {
	...
	err := r.db.QueryRowContext(ctx, query, id).Scan(&cat.ID, &cat.PropertyID, &cat.Name, &cat.Description, &cat.RentAmount, &cat.QuantityAvailable, &cat.Photos, &cat.VideoURL, &cat.CreatedAt)
	if errors.Is(err, sql.ErrNoRows) {
...
func (r *PostgresRepo) GetCategoriesByPropertyID(ctx context.Context, propertyID uuid.UUID) ([]domain.UnitCategory, error) {
	...
	for rows.Next() {
		...
		if err := rows.Scan(&cat.ID, &cat.PropertyID, &cat.Name, &cat.Description, &cat.RentAmount, &cat.QuantityAvailable, &cat.Photos, &cat.VideoURL, &cat.CreatedAt); err != nil {
			return nil, err
		}
...
func (r *PostgresRepo) UpdateCategory(ctx context.Context, cat *domain.UnitCategory) error {
	...
	_, err := r.db.ExecContext(ctx, query, cat.Name, cat.Description, cat.RentAmount, cat.QuantityAvailable, cat.Photos, cat.VideoURL, cat.ID)
	return err
}
...
func (r *PostgresRepo) GetAllLandlordProfiles(ctx context.Context) ([]domain.AgentView, error) {
	query := `
		SELECT lp.id, lp.user_id, u.email, u.phone, COALESCE(lp.full_name, ''), lp.national_id_number,
	...
	var list []domain.AgentView
	for rows.Next() {
		var a domain.AgentView
		if err := rows.Scan(&a.ID, &a.UserID, &a.Email, &a.Phone, &a.FullName, &a.NationalIDNumber,
